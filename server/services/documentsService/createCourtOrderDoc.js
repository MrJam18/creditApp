...
//
//
// module.exports = async function createCourtOrderDoc(courtId, contract, date, isContractJur, ignorePayments, agentId) {
//     // const court = await Courts.findByPk(courtId,{ include: {all: true}, plain: true});
//     // const agent = await Agents.findByPk(agentId, {plain: true});
//     // const cession = contract.cession;
//     // const {firstCreditorName, cessionText, cessionDocuments} = await getTextFromCessions(cession, contract.organization.name);
...
//     // const limited = countLimited(result, contract.sum_issue, contract.date_issue);
//     const genitiveDebtor = incline({first: debtor.name, last: debtor.surname, middle: debtor.patronymic}, 'genitive');
//     result.fee = countFeeOrder(result.sum);
...
//     console.log(typeof cession.text);
...
//             new Header('Заявление о выдаче судебного приказа.'),
//             new Indent(`${issuedDate} г. ${firstCreditorName} (далее - "Заимодавец") и ${debtor.fullName} (далее - "Должник") заключили договор займа № ${contract.number} (далее - "Договор"). В соответствии с условиями договора заимодавец передал должнику денежную сумму в размере ${contract.sum_issue} рублей сроком на ${result.loanDays} ${result.loanDaysName}, а Должник обязался возвратить такую же сумму денег и уплатить проценты в размере ${contract.percent} % годовых от суммы займа.
//             `),
//             new Indent(`${issuedDate} г. должник денежную сумму по договору получил, что подтверждается расходным кассовым ордером.`),
//             new Indent(`До настоящего времени сумма займа не оплачена. Этим Заемщик нарушил обязанность возвратить сумму займа до ${changeDateFormat(contract.due_date)} г., предусмотренную условиями договора.`),
...
//         content.children.splice(34, 0, ...result.limitedText)
//     }
//     const doc = new Document({
//         creator: 'creditApp',
//         sections: [content, countingTables(contract, result.breaks, agent.surnameAndInitials)]
//     })
//     return doc;
//
// }