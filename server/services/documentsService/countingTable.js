...
function percentTable(contract, breaks) {
    ...
    breaks.forEach((el, index)=> {
        if(breaks[index + 1]){
            ...
            const percentsInPeriod = new Number(breaks[index + 1].percents - percents).toFixed(2);
        ...
        breaks.forEach((el, index)=>{
            if(breaks[index + 1]){
                if(el.payment){
                        ...
                        const penaltiesInPeriod = new Number(breaks[index + 1].penalties - penalties).toFixed(2);
