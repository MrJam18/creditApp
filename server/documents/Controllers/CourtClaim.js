...
module.exports = class CourtClaim extends ContractDocument
{
    ...
    async #prepareData(agentId, courtId, contractId, groupId, countDate, options, type)
    {
        ...
        if(creditor.creditorTypeId === 3) {
            ...
            const namesArray = creditor.name.split(' ', 3);
            if(namesArray?.length === 3) {
                const creditorGenitive = incline({first: namesArray[1], last: namesArray[0], middle: namesArray[2]}, 'genitive');
                creditor.genitive = `${creditorGenitive.last} ${creditorGenitive.first} ${creditorGenitive.middle}`;
            }
