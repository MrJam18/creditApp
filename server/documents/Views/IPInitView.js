...
{
    constructor(bailiff, creditor, agent, debtor, courtName, executiveDoc)
    {
        const options = {
            bigText: true
        }
    ...
    #createBody()
    {
        ...
        body.addIndentRow(`Прошу принять ${this.executiveDoc.type} № ${this.executiveDoc.number} от ${this.executiveDoc.dateIssue} г., выданный судом: ${this.courtName} по гражданскому делу о взыскании задолженности с ${genitiveDebtor.last} ${genitiveDebtor.first} ${genitiveDebtor.middle} в пользу ${this.creditor.name} по договору займа в общем размере ${this.executiveDoc.sum} руб. Из них:`);
