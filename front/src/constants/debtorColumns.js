...
const passportColumns = [new Column('тип паспорта', 'type', 'selected', {func: getPassportTypes}), new Column('серия и номер', 'seriesAndNumber', 'composed', [new Column('серия паспорта', 'series'), new Column('номер паспорта', 'number')]), new Column('дата выдачи', 'issued_date', 'date'), new Column('Кем выдан', 'issued_by', undefined, {fullWidth}),  new Column("код подразделения", 'gov_unit_code'), new Column('дата обновления', 'updatedAt')];
